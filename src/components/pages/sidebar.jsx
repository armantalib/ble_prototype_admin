...
import { childdark, childlight, coursedark, courselight, dashboarddark, dashboardlight, finabeelight, parentdark, parentlight, quizdark, quizlight,logo } from '../icons/icon';
        ...
        <>
            {isLogin ? (
                <div className='flex h-screen min-h-screen'>
                            ...
                            <div className='scrolbar' style={{ display: 'flex', flexDirection: 'column', overflowY: 'auto', height: '100%', paddingTop: '1rem' }}>
                                <div className='mb-5 flex items-center justify-center'>
                                    <button onClick={() => { navigate('/dashboard') }} className="">
                                        <img style={{ height: '4rem', width: 'auto' }} src={logo} className='' alt="" />
                                    </button>
                                </div>
                                            ...
                                            <Fragment key={i}>
                                                {item.subItems ? (
                                                    <SubMenu icon={<MdProductionQuantityLimits style={{ height: 'auto', width: "25px", marginRight: '10px' }} />} label={item.items} className={`w-full plusJakara_semibold text_secondary rounded-3 mb-2`}>
                                                        {item.subItems.map((subItem, j) => (
                                                            <MenuItem
                                                                ...
                                                                className={`w-full rounded-3 mb-2 ${isChildPath(subItem.path, location.pathname) ? 'bg_darkprimary text_white plusJakara_semibold' : 'text_secondary'}`}
                                                            >
                                                    ...
                                                    <MenuItem
                                                        ...
                                                        className={`w-full rounded-3 mb-2 ${isChildPath(item.path, location.pathname) ? 'bg_darkprimary text_white plusJakara_semibold' : 'text_secondary'}`}
                                                    >
                                                        <div className="flex items-center gap-4">
                    ...
                    <main className='w-full overflow-auto' style={{ backgroundColor: '#f6f6f6' }}>
                        {children}
                    </main>
                </div>
            ) : (
